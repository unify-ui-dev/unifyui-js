import { appendBefore, areValidHTMLElements, find, findAll } from '@unifyui-js/dom_util'
import { ModalOptions } from "./types";

/**
 * Toggles the state of the modal (open or close).
 * @param modalElement - The modal element.
 * @param modalContent - The content element within the modal.
 * @param action - The action to perform, either 'open' or 'close'.
 */
...
const setBodyScrollable = (enableStackedModals_:boolean, allowBodyScroll_:boolean, modalElement:HTMLElement) => {
    ...
    const filteredShown = shownModalElementsWithoutBodyScroll.filter((element) => element !== modalElement)
 ...
 * @param modalElement - The modal element.
 * @param triggerButton - The button that triggers the modal.
 * @param options - Options for modal behavior.
 */
const initModal = (modalElement: HTMLElement, triggerButton: HTMLElement | null, options: ModalOptions) => {
    ...
    const { animateContent, allowBodyScroll, preventCloseModal, overlayClass, onShow, onHide, enableStackedModals } = options;
